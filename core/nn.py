...
def ResUnit_tail(input_tensor, feed_dict, shape_dict, var_dict=None):
    ...
    with tf.variable_scope('conv1'):
        CONV_out1 = conv_dilate_layer(RELU_out1, feed_dict, 12,
                                      'SAME', shape_dict[0], var_dict)
    with tf.variable_scope('bias1'):
        BIAS_out1 = bias_layer(CONV_out1, feed_dict, shape_dict[0][3], var_dict)
    RELU_out2 = ReLu_layer(BIAS_out1)
    # The second dilated conv layer
    with tf.variable_scope('conv2'):
        CONV_out2 = conv_dilate_layer(RELU_out2, feed_dict, 12,
                                      'SAME', shape_dict[1], var_dict)
...
def grad_convs(input_tensor, feed_dict, shape, var_dict=None):
...
def grad_norm(input_tensor, feed_dict, shape, var_dict=None):
...
def BN(input_tensor, feed_dict=None, bn_scope=None,is_train=False, shape=None,
    ...
    BN_out = tf.layers.batch_normalization(inputs=input_tensor,
                                          ...
                                          training=bn_training, # using current batch statistics or moving statistics
                                          trainable=bn_trainable, # Depends on whether train or not
...
def bias_layer(input_tensor, feed_dict, shape, var_dict):

    scope_name = tf.get_variable_scope().name
    print('Layer name: %s'%scope_name)
    bias = get_bias(feed_dict, shape)

    bias_out = tf.nn.bias_add(input_tensor, bias)
    if var_dict is not None:
        if not var_dict.has_key(scope_name):
            var_dict[scope_name] = {}
